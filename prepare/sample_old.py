...
class Sample_old(object):

    def __init__(self, context, spk_id, adr_id, responses, label, n_agents_in_ctx, max_n_agents, max_n_words,
                 test=False):
        ...
        self.response_label = get_response_label(self.response, self.true_res, test)

        ####################
        # Addressee labels #
        ####################
        self.true_adr = get_adr_label(adr_id, self.agent_index_dict)  # 不是ID，而是改成在spk_agent中对应的下标
...
def get_response_label(response, true_label, test):
    if test:
        return true_label
    label = [0 for res in range(len(response))]
    label[true_label] = 1
    return label
...
def get_adr_agent_one_hot_vec(context, agent_index_dict, max_n_agents):
    ...
    for c in context:
        vec = [0 for i in xrange(max_n_agents)]
        adr_id = c[2]
        if adr_id in agent_index_dict:
            vec[agent_index_dict[adr_id]] = 1
        speaking_agent_one_hot_vector.append(vec)
    return speaking_agent_one_hot_vector
...
def padding_response1(response, max_n_words):
    pads = []
    diff = max_n_words - len(response)
    pad = [0 for i in xrange(diff)]
    pad.extend(response)
    pads.append(pad)
    return pads
...
def padding_context(context, max_n_words):
    ...
    for sent in context:
        pad_context = padding_sent(sent[-1])
        pad_context_list.append(pad_context)
    return pad_context_list
