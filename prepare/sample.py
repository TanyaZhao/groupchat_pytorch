...
class Sample(object):
    ...
    def __init__(self, context, spk_id, adr_id, responses, label, n_agents_in_ctx, max_n_agents, max_n_words, pad=True,
        ...
        print self.spk_agents
        for i in self.spk_agents:
            if i >= 5:
                print "6666"
        ...
        self.n_agents_in_lctx = len(set([c[1] for c in context] + [spk_id]))  # n_agents_in_limit_context
...
def get_adr_agent_one_hot_vec(context, agent_index_dict,max_n_agents):
    adr_agent_one_hot_vector = []
    for c in context:
        vec = [0 for i in xrange(max_n_agents)]
        adr_id = c[2]
        if adr_id != '-':
            vec[agent_index_dict[adr_id]] = 1
        adr_agent_one_hot_vector.append(vec)
    return adr_agent_one_hot_vector
